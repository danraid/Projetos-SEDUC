# -*- coding: utf-8 -*-
"""quadro_horários_ver_2_8de_28_02_25.py.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/10nffqA05NEvlAjz2pykGzpj-_JT1ERrF
"""
...
tempos = [1, 2, 3, 4, 5]
NUM_ITERACOES_HEUR = 1000    # iterações da busca heurística inicial
NUM_ITERACOES_REFIN = 1000    # iterações do ajuste fino
N_BEST = 10                 # número de melhores soluções para refinamento
RL_ITER = 1000               # iterações do loop de RL para refinar candidatos
...
def ajuste_fino(horario, nao_alocados):
    """
    Para cada pedido (ped) que ainda possui horas remanescentes (ch_rest) não alocadas,
    tenta, para cada dia (na ordem de dias_semana) onde o professor ainda não foi alocado para a turma,
    alocar nos tempos (de 1 a 5) chamando pode_alocar para revalidar todas as restrições.
    O valor ch_rest é decrementado a cada alocação, garantindo que não seja ultrapassado o CH.TURMA.
    Retorna a estrutura de horário atualizada e a lista de pedidos que ainda não foram totalmente alocados.
    """
    novos_nao = []
    for ped, ch_rest in nao_alocados:
        # Percorre os dias em ordem fixa para tentar alocar os tempos restantes
        for d in dias_semana:
            if ch_rest <= 0:
                break
...
best_refined, best_nao_alocados, best_score_ref, feats_refin = refined_solucoes[0]

print("Resultado após ajuste fino (refinamento das N melhores soluções):")
print(f"Melhor Score Refinado: {best_score_ref}")
usados_final = conta_alocacoes(best_refined)
...
# =========================================================================
# 6F) INTEGRAÇÃO COM APRENDIZADO POR REFORÇO (RL)
#    Nesta etapa, usamos as features e os scores para guiar a seleção dos candidatos.
#    Aqui implementamos um loop RL simples que, para cada candidato dos N melhores,
#    aplica uma ação (mais refinamento) e observa a recompensa (variação no score).
# =========================================================================
# Definindo uma função para discretizar o estado (por exemplo, usando percent_filled e nao_alocados)
def discretiza_estado(feats):
    # Vamos discretizar percent_filled em 10 faixas e nao_alocados em faixas de 10
    pf_bin = int(feats["percent_filled"] // 10)
    nao_bin = int(feats["nao_alocados"] // 10)
    return (pf_bin, nao_bin)

# Inicializa uma "tabela Q" simples (dicionário) para os estados
Q_table = defaultdict(float)

def rl_refinamento(candidatos, iter_rl):
    refined_candidates = []
    # Para cada candidato, aplicamos ações de refinamento e atualizamos Q-values
    for hor, nao_aloc, score_init, feats_init in candidatos:
        state = discretiza_estado(feats_init)
        best_candidate = (hor, nao_aloc, score_init, feats_init)
        # Loop RL: cada ação é "aplicar uma iteração extra de ajuste fino"
        for _ in range(iter_rl):
            hor_temp, nao_temp = ajuste_fino(hor, nao_aloc)
            score_temp, feats_temp = score_solucao(hor_temp, demanda)
            reward = score_temp - score_init  # recompensa é melhoria no score
            # Atualiza Q_table para o estado (uma forma simples de média acumulada)
            Q_table[state] = 0.5 * Q_table[state] + 0.5 * reward
            # Se a melhoria foi positiva, atualizamos a melhor solução para este candidato
            if score_temp > score_init:
                best_candidate = (hor_temp, nao_temp, score_temp, feats_temp)
                score_init = score_temp
                feats_init = feats_temp
            # Atualiza o estado
            state = discretiza_estado(feats_init)
        refined_candidates.append(best_candidate)
    return refined_candidates

# Aplica o RL para refinar os N melhores candidatos
rl_candidates = rl_refinamento(best_solucoes, RL_ITER)
rl_candidates.sort(key=lambda x: x[2], reverse=True)
best_rl_solution, best_rl_nao, best_rl_score, best_rl_feats = rl_candidates[0]

print("Resultado após integração RL:")
print(f"Melhor Score Refinado (RL): {best_rl_score}")
usados_final = conta_alocacoes(best_rl_solution)
total_final = total_slots_ideal(demanda)
percent_filled = (usados_final / total_final) * 100 if total_final else 0
print(f"Slots usados: {usados_final}")
print(f"Slots vazios: {total_final - usados_final}")
...
# =========================================================================
# 6G) SALVAR FEATURES EXTRAÍDAS (para uso em ML futuro)
# =========================================================================
...
dfs_turma = gera_df_por_turma(best_rl_solution)
dfs_prof = gera_df_por_professor(best_rl_solution)
...
vazios_detalhe = []
for tnr in best_rl_solution:
    for niv in best_rl_solution[tnr]:
        for d in dias_semana:
            for (ser, tur), dic in best_rl_solution[tnr][niv][d].items():
                for tm in tempos:
                    if tm not in dic:
