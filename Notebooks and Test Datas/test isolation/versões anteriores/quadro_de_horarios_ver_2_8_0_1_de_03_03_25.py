# -*- coding: utf-8 -*-
"""Quadro de Horarios ver.2.8.0.1 de 03.03.25.ipynb
...
NUM_ITERACOES = 10000   #2.8.0.0 - Vamos seguir a mesma estrutura do ver.2.1 (múltiplos builds)
NUM_ITERACOES_SA = 1000 #2.8.0.0
...
# =========================================================================
# CÓDIGO PARA CALCULAR A PORCENTAGEM DE PROFESSORES ALOCADOS
# =========================================================================

# Conjunto de todos os professores que aparecem na demanda
professores_total = set(p.prof for p in demanda)

# Identifica professores efetivamente alocados em algum slot
professores_alocados = set()
for tnr in melhor_horario:
    for niv in melhor_horario[tnr]:
        for d in dias_semana:
            for (_, _), dic in melhor_horario[tnr][niv][d].items():
                for _, (prof, _) in dic.items():
                    professores_alocados.add(prof)

# Calcula a porcentagem de professores alocados
if len(professores_total) > 0:
    perc_prof_alocados = 100 * len(professores_alocados) / len(professores_total)
else:
    perc_prof_alocados = 0

print(f"Porcentagem de professores alocados na solução inicial: {perc_prof_alocados:.2f}%")
...
def gera_vizinhos_tabu(horario, nao_alocados, demanda):  #2.8.0.0
    ...
    for (tnr, niv, d, ser, tur, tm, prof, disc) in alocados:
        ...
        if new_nao:
            ...
            dd = copy.deepcopy(dias_semana)
            random.shuffle(dd)
            tt = copy.deepcopy(tempos)
            random.shuffle(tt)
            for ddx in dd:
                if ch_rest <= 0:
                    break
                for ttx in tt:
                    if ch_rest <= 0:
                        break
                    if pode_alocar(new_hor, pedN, ddx, ttx):
                        alocar_1tempo(new_hor, pedN, ddx, ttx)
                        ch_rest -= 1
                        break
            if ch_rest == 0:
                new_nao.pop(idx)
