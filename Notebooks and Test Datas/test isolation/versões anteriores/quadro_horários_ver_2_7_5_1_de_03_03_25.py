# -*- coding: utf-8 -*-
"""QUADRO_HORÁRIOS_VER_2_7_5_1 DE_03_03_25.ipynb
...
def score_solucao_com_penalidades(horario, demanda):
    ...
    # 2.7.5.1: Aumentado o peso para CH não alocadas de 150 para 250 para incentivar o preenchimento
    score = usados - peso_gap_turma * gaps_turma - 250 * nao_aloc - penalidade_soft   # 2.7.5.1
    feats = {
        "usados": usados,
        "slots": slots,
        "gaps": gaps_turma,
...
def alocar_demanda(horario, demanda):
    nao_alocados = []
    # 2.7.5.1: Ordena os pedidos por carga horária (maiores primeiro) para maximizar preenchimento
    dem_local = sorted(demanda, key=lambda p: p.ch, reverse=True)  # 2.7.5.1
    for ped in dem_local:
        ch_rest = ped.ch
        # 2.7.5.1: Itera em ordem fixa dos dias para preencher os slots
        for d in dias_semana:  # 2.7.5
            # 2.7.5.1: Itera em ordem fixa dos tempos
            for t in tempos:  # 2.7.5
                if ch_rest <= 0:
                    break
