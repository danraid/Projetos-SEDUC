# -*- coding: utf-8 -*-
"""QUADRO_HORÁRIOS_VER_2_7_5_5 DE_03_03_25.ipynb
...
#2.7.5.4 - limite de tentativas para atingir 60% na solução heurística
MAX_HEUR_TENTATIVAS = 1000  #2.7.5.4

#2.7.5.5 - Parâmetros do Tabu Search
TABU_ITERS = 300             #2.7.5.5 - Iterações do Tabu
TABU_TENURE = 30             #2.7.5.5 - Tempo de proibição de cada movimento
...
# =========================================================================
# 6A) GERAÇÃO DE SOLUÇÃO INICIAL (MÉTODO HEURÍSTICO) #2.7.5.3
# =========================================================================
def alocar_demanda_heuristico(horario, demanda):  #2.7.5.3
    #2.7.5.3 - Método simples: ordena por maior CH e tenta alocar sequencialmente
    nao_alocados = []
    dem_local = sorted(demanda, key=lambda p: p.ch, reverse=True)  #2.7.5.3
    for ped in dem_local:
        ch_rest = ped.ch
        for d in dias_semana:     #2.7.5.3 - agora sem embaralhar
            if ch_rest <= 0:
                break
            for t in tempos:      #2.7.5.3 - iterando do tempo 1 a 5
                if ch_rest <= 0:
                    break
...
def constroi_solucao_heuristica(demanda):  #2.7.5.3
    hor_temp = cria_horario_vazio(demanda)
    nao_aloc = alocar_demanda_heuristico(hor_temp, demanda)  #2.7.5.3
...
# =========================================================================
# 6C) CONSTRUÇÃO HEURÍSTICA + SA, mas agora garantimos >= 60% #2.7.5.4
# =========================================================================

#2.7.5.4 - Tentamos várias vezes construir a solução heurística até atingir 60% ou esgotar tentativas
best_hor_inicial = None  #2.7.5.4
best_nao_aloc_inicial = None  #2.7.5.4
best_percent_inicial = 0  #2.7.5.4

for _ in range(MAX_HEUR_TENTATIVAS):  #2.7.5.4
    hor_temp, nao_aloc_temp = constroi_solucao_heuristica(demanda)  #2.7.5.4
    used_temp = conta_alocacoes(hor_temp)  #2.7.5.4
    slots_temp = total_slots_ideal(demanda)  #2.7.5.4
    percent_temp = (used_temp / slots_temp) * 100 if slots_temp else 0  #2.7.5.4
    if percent_temp > best_percent_inicial:  #2.7.5.4
        best_percent_inicial = percent_temp  #2.7.5.4
        best_hor_inicial = hor_temp  #2.7.5.4
        best_nao_aloc_inicial = nao_aloc_temp  #2.7.5.4
    if best_percent_inicial >= 60:  #2.7.5.4
        break  #2.7.5.4

#2.7.5.4 - Agora best_hor_inicial e best_nao_aloc_inicial são a melhor heurística >= 60% ou a melhor obtida
score_inicial, feats_inicial = score_solucao_com_penalidades(best_hor_inicial, demanda)  #2.7.5.4
print("#2.7.5.4 - Resultado inicial do método heurístico (>=60% ou melhor obtido):")  #2.7.5.4
print(f"#2.7.5.4 - Score inicial: {score_inicial}")  #2.7.5.4
usados_inicial = feats_inicial["usados"]  #2.7.5.4
total_inicial = feats_inicial["slots"]    #2.7.5.4
percent_inicial = feats_inicial["percent_filled"]  #2.7.5.4
print(f"#2.7.5.4 - Slots usados: {usados_inicial}")  #2.7.5.4
print(f"#2.7.5.4 - Slots vazios: {total_inicial - usados_inicial}")  #2.7.5.4
print(f"#2.7.5.4 - Percentual de preenchimento: {percent_inicial:.2f}%")  #2.7.5.4
print("----------------------------------------------")
...
#2.7.5.3 -> agora usando a solução inicial que garantiu >= 60% (ou melhor)
hor_final, nao_aloc_final = simulated_annealing(
    best_hor_inicial, best_nao_aloc_inicial, demanda, NUM_ITERACOES_SA, T0=100.0, alpha=0.99
)  #2.7.5.4

score_final, feats_final = score_solucao_com_penalidades(hor_final, demanda)
print("Resultado após refinamento com SA:")
print(f"Melhor Score (Heur >=60% + SA): {score_final}")
usados_final = feats_final["usados"]
total_final = feats_final["slots"]
percent_filled = feats_final["percent_filled"]
...
#2.7.5.4 - corrigir inicialização de CH_alocada_por_servidor
CH_alocada_por_servidor = {}  #2.7.5.4
    ...
    for niv in hor_final[tnr]:
        for d in dias_semana:
            for (serie, turma), dic in hor_final[tnr][niv][d].items():
                for t in dic:
                    prof, _ = dic[t]
                    CH_alocada_por_servidor[prof] = CH_alocada_por_servidor.get(prof, 0) + 1  #2.7.5.4
...
# =========================================================================
# 6E) TABU SEARCH (NOVA ABORDAGEM) #2.7.5.5
# =========================================================================
#2.7.5.5 - Vamos propor um Tabu Search após o SA para tentar chegar a >80%
def gera_vizinhos_tabu(horario, nao_alocados, demanda):  #2.7.5.5
    """
    Gera vizinhos removendo 1 tempo alocado e adicionando 1 hora
    de algum pedido não alocado, semelhante ao SA, mas de forma sistemática.
    Retorna uma lista de (novo_hor, novo_nao).
    """  #2.7.5.5
    vizinhos = []  #2.7.5.5
    # Tentamos remover um slot e alocar 1 pedido do nao_alocados
    alocados = []
    for tnr in horario:
        for niv in horario[tnr]:
            for d in dias_semana:
                for (ser, tur), dic in horario[tnr][niv][d].items():
                    for tm, (prof, disc) in dic.items():
                        alocados.append((tnr, niv, d, ser, tur, tm, prof, disc))
    if not alocados:
        return vizinhos  #2.7.5.5

    # Aleatoriamente, escolhemos um subconjunto menor para não explodir combinatorial
    random.shuffle(alocados)  #2.7.5.5
    alocados = alocados[:5]   #2.7.5.5 - limitamos a 5 possíveis remoções

    # Para cada tempo removido, tentamos realocar 1 pedido do nao_alocados
    for (tnr, niv, d, ser, tur, tm, prof, disc) in alocados:
        ...
        # gera 1 "pedido" devolvido
        pedido_falso = Pedido(prof=prof, turno=tnr, nivel=niv, serie=ser, turma=tur, disc=disc, ch=1)
        new_nao.append((pedido_falso, 1))

        # Tenta alocar 1 pedido do nao_alocados
        if new_nao:
            idx = random.choice(range(len(new_nao)))
            pedN, ch_rest = new_nao[idx]
            ...
            for dd in dias_rand:
                if ch_rest <= 0:
                    break
                for tt in tempos_rand:
                    if ch_rest <= 0:
                        break
                    if pode_alocar(new_hor, pedN, dd, tt):
                        alocar_1tempo(new_hor, pedN, dd, tt)
                        ch_rest -= 1
                        break
            if ch_rest == 0:
                new_nao.pop(idx)
            else:
                new_nao[idx] = (pedN, ch_rest)

        vizinhos.append((new_hor, new_nao))  #2.7.5.5
    return vizinhos  #2.7.5.5

def tabu_search(horario, nao_alocados, demanda, iters=TABU_ITERS, tenure=TABU_TENURE):  #2.7.5.5
    """
    Tabu Search: para cada iteração:
    - Gera vizinhos
    - Escolhe o melhor não tabu
    - Atualiza a lista tabu (movimentos ou states)
    """  #2.7.5.5
    current_hor = copy.deepcopy(horario)
    current_nao = copy.deepcopy(nao_alocados)
    current_score, _ = score_solucao_com_penalidades(current_hor, demanda)
    ...
    # Guardamos states (hash do horario) ou moves como tabu
    # aqui guardamos o hash do set de alocações:
    def state_hash(h, n):
        # soma de (prof, d, t)...
        # para simplificar, pegamos o hash da string das alocações
        ...
        items.sort()
        return str(items) + str(n)  #2.7.5.5

    tabu_list = {}
    it = 0
    while it < iters:
        it += 1
        neighbors = gera_vizinhos_tabu(current_hor, current_nao, demanda)  #2.7.5.5
        best_local = None
        best_local_score = -9999999
        chosen_move = None
        ...
        if best_local is None:
            # se todos vizinhos são tabu e piores que best_score, paramos
            break

        # aplicamos best_local
...
# =========================================================================
# 7) RODA SA + TABU #2.7.5.5
# =========================================================================
hor_sa, nao_sa = simulated_annealing(
    best_hor_inicial, best_nao_aloc_inicial, demanda, NUM_ITERACOES_SA, T0=200.0, alpha=0.98
)  #2.7.5.5

score_sa, feats_sa = score_solucao_com_penalidades(hor_sa, demanda)  #2.7.5.5
print("#2.7.5.5 - Resultado após SA intermediário:")
print(f"#2.7.5.5 - Score SA: {score_sa}")
print(f"#2.7.5.5 - FillRate SA: {feats_sa['percent_filled']:.2f}%")

#2.7.5.5 - Agora rodamos Tabu Search a partir do SA
hor_tabu, nao_tabu = tabu_search(hor_sa, nao_sa, demanda, iters=TABU_ITERS, tenure=TABU_TENURE)  #2.7.5.5
score_final, feats_final = score_solucao_com_penalidades(hor_tabu, demanda)  #2.7.5.5

print("#2.7.5.5 - Resultado final após SA + TabuSearch:")
print(f"#2.7.5.5 - Score final: {score_final}")
print(f"#2.7.5.5 - FillRate final: {feats_final['percent_filled']:.2f}%")

hor_final = hor_tabu  #2.7.5.5
nao_aloc_final = nao_tabu  #2.7.5.5
...
dfs_turma = gera_df_por_turma(hor_final)
dfs_prof = gera_df_por_professor(hor_final)
