# -*- coding: utf-8 -*-
"""QUADRO_HORÁRIOS_VER_2_7_2 DE_29_02_25.ipynb
...
tempos = [1, 2, 3, 4, 5]
NUM_ITERACOES_HEUR = 10000    # número de iterações da busca inicial (GRASP)
NUM_ITERACOES_SA = 10000      # número de iterações do Simulated Annealing (SA)
...
def check_professor_gap(horario, prof, turno, dia):
    ...
    """
    allocated_times = []
    # Percorre todos os níveis (geralmente, o professor atua apenas num turno)
    ...
    if len(allocated_times) <= 1:
        return False  # Com uma aula ou nenhuma, não há gap
    allocated_times = sorted(allocated_times)
    # Verifica se há intervalos maiores que 1 entre horários consecutivos
...
# =========================================================================
# 6A) GERAÇÃO DE SOLUÇÕES INICIAIS (GRASP)
# =========================================================================
...
def simulated_annealing(horario, nao_alocados, demanda, num_iter, T0=500.0, alpha=0.999):
    ...
    current_score, _ = score_solucao(current_hor, demanda)
    ...
    for i in range(num_iter):
        ...
        # Escolhe aleatoriamente um pedido não alocado para tentar melhorar
        if new_nao:
            idx = random.choice(range(len(new_nao)))
            ped, ch_rest = new_nao[idx]
            allocated = False
            for d in random.sample(dias_semana, len(dias_semana)):
                if ch_rest <= 0:
                    break
                for t in random.sample(tempos, len(tempos)):
                    if ch_rest <= 0:
                        break
        ...
        new_score, _ = score_solucao(new_hor, demanda)
        delta = new_score - current_score
        # Aceita solução melhor ou pior com probabilidade exp(delta/T)
...
for hor, nao_aloc, score_init, feats_init in best_solucoes:
    ...
    score_ref, feats_ref = score_solucao(new_hor, demanda)
